...
fn test_01_01_sqlite_wal_init_and_schema() {
    ...
    assert_eq!(
        foreign_keys, 1,
        "Les clés étrangères doivent être activées (foreign_keys = ON)"
    );
    ...
    assert!(
        ...
        "La table 'files' doit exister"
    );
    assert!(
        tables.contains(&"chunks".to_string()),
        "La table 'chunks' doit exister"
    );
    assert!(
        tables.contains(&"fts_notes".to_string()),
        "La table virtuelle 'fts_notes' doit exister"
    );
    ...
    assert!(
        triggers.contains(&"chunks_ai".to_string()),
        "Le trigger 'chunks_ai' doit exister"
    );
    assert!(
        triggers.contains(&"chunks_ad".to_string()),
        "Le trigger 'chunks_ad' doit exister"
    );
    assert!(
        triggers.contains(&"chunks_au".to_string()),
        "Le trigger 'chunks_au' doit exister"
    );
}
...
"#;
    ...
    assert_eq!(
        frontmatter.source_media,
        Some("Attachments/meeting_20260912.wav".to_string())
    );

    assert!(
        body.starts_with("# Introduction"),
        "Le corps doit commencer par le titre sans le frontmatter"
    );
...
Corps minimal.
"#;
    let (minimal_frontmatter, minimal_body) =
        parse_markdown(minimal_content).expect("Échec parsing minimal");
...
fn test_01_03_indexing_bm25_search_and_cascade_delete() {
    ...
    let results = storage
        .search_fts("souveraine", 10)
        .expect("Échec recherche FTS");
    assert_eq!(results.len(), 1, "La recherche doit trouver 1 résultat");
    assert_eq!(results[0].chunk_id, "chunk_arch_01");
    assert!(
        results[0].snippet.contains("souveraine") || results[0].snippet.contains("<mark>"),
        "Le snippet doit être généré"
    );

    // 2. Suppression du chunk et vérification du trigger chunks_ad
    storage
        .delete_chunk("chunk_arch_01")
        .expect("Échec suppression chunk");

    let fts_count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM fts_notes WHERE chunk_id = ?1",
            ["chunk_arch_01"],
            |row| row.get(0),
        )
    ...
    assert_eq!(
        fts_count, 0,
        "Le trigger chunks_ad doit avoir supprimé le chunk de fts_notes"
    );

    let post_delete_results = storage
        .search_fts("souveraine", 10)
        .expect("Échec recherche post suppression");
    assert!(
        post_delete_results.is_empty(),
        "La recherche FTS ne doit plus retourner de résultat après suppression"
    );
}
...
fn test_01_04_vault_path_resolution_priority() {
    ...
    assert_eq!(
        ...
        "La priorité doit être accordée à JEANNE_VAULT_PATH"
    );
    ...
    assert!(
        ...
        "Le fallback doit se terminer par JeanneVault : {:?}",
        fallback_resolved
    );
}

#[test]
fn test_01_05_fts_xss_protection_and_query_resilience() {
    let dir = tempdir().expect("tempdir");
    let db_path = dir.path().join("xss_test.db");
    let storage = StorageManager::open(&db_path).expect("open");
    storage.init_schema().expect("init schema");

    storage
        .upsert_file("notes/xss.md", "hash123", 1700000000, None)
        .expect("upsert file");
    ...
    // 1. Vérification de l'échappement XSS dans le snippet
    let results = storage.search_fts("motclef", 5).expect("search");
    assert_eq!(results.len(), 1);
    let snippet = &results[0].snippet;

    assert!(
        snippet.contains("&lt;script&gt;alert(&#39;xss&#39;)&lt;/script&gt;")
            || snippet.contains("&lt;script&gt;"),
        "Le snippet doit avoir échappé les balises script : {snippet}"
    );
    assert!(
        !snippet.contains("<script>"),
        "Le snippet ne doit PAS contenir de balise script non échappée"
    );
    assert!(
        snippet.contains("<mark>motclef</mark>"),
        "Le terme recherché doit être surligné avec <mark> : {snippet}"
    );

    // 2. Vérification de la résilience aux erreurs de syntaxe FTS5 (guillemets orphelins, opérateurs)
    let unbalanced_results = storage.search_fts("motclef \" unclosed", 5);
    assert!(
        unbalanced_results.is_ok(),
        "La recherche avec guillemet orphelin ne doit pas échouer"
    );
}
